...
const INITIAL = {
 ...
 "]"(control) {
  ...
  control.stack.pop();
  control.block =
   control.stack[control.stack.length - 1];
  control.statement =
   control.statementStack.pop();
 },
};
  ...
  queueToStatement() {
   if (control.queue.length) {
    switch (control.queue) {
     ...
     default:
      if (
       String(parseFloat(control.queue)) ===
       control.queue
      ) {
       control.statement.push(
...
function smartJoin(basePath, filePath) {
 ...
 const relativeTo = params.get("key");
 const relativeSegments = relativeTo.split("/");
 relativeSegments.pop();
 const relativeDir = relativeSegments.join("/");
 ...
 const me = {
  ...
  async each(callbackCrown) {
   ...
   for (
    ...
    result.push(
     uncrown(
      await callback(currentValue[index], index)
     )
    );
   }
   currentValue = result;
  ...
  async find(argumentCrown) {
   ...
   for (let i = 0; i < currentValue.length; i++) {
    if (
     await uncrown(
      await callback(currentValue[i], i)
     )
    ) {
       ...
       try {
        ...
        const fileModule = eval(`(
         function () {
          ...
          await scope.walk(${JSON.stringify(
           code
          )})
          return scope
         }})()`);
  ...
  async promise(handler) {
   try {
    currentValue = await new Promise(
     uncrown(handler)
    );
   } catch (e) {
    currentError = e;
