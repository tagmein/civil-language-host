...
const ROOT_DIR = dirname(import.meta.url).replace(
 "file://",
 ""
);
...
async function main() {
 const indexHtml = (
  await readFile(join(ROOT_DIR, "index.html"))
 ).toString("utf-8");
 const languageCrown = (
  await readFile(
   join(ROOT_DIR, "language", "crown.js")
  )
 ).toString("utf-8");
 ...
 const httpServer = http.createServer(
  async function (request, response) {
   try {
    ...
    const [requestPath, requestParamString] =
     request.url.split("?");
    const requestParams = querystring.parse(
     requestParamString ?? ""
    );
    console.log({
     ...
     case "POST": {
      ...
      if (requestBody.key !== requestParams.key) {
       ...
       response.end(
        JSON.stringify({
         error:
          "request body key must match request parameter key",
        })
       );
       return;
      }
      if (
       requestBody.mode !== requestParams.mode
      ) {
       response.statusCode = 400;
       response.end(
        JSON.stringify({
         error:
          "request body mode must match request parameter mode",
        })
       );
       return;
      }
...
main().catch(async function (e) {
 ...
 const errorFilePath = join(
  STORAGE_DIR,
  ".civil-memory-kv-error.txt"
 );
 await writeFile(errorFilePath, e.stack);
 console.log(
  `Error details written to ${errorFilePath}`
 );
 process.exitCode = 1;
});
